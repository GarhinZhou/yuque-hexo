...
updated: '2025-09-24 14:30:37'
---
## 回顾总结
几个月没做过堆题了...基础的东西都有点忘了，加上之前没完全弄清楚，再简单整理一下

**tcachechunk**：是64个单链表结构的bins，每个bins最大存放7个对应大小的chunk，chunk的大小在64位中 0x10 字节递增，从 0x18 到 0x408 字节，在32位机器上以8字节递增，从 0xc 字节到 0x200 字节

**fastchunk**：只有fd指针，大小从0x20-0x80（包括prev_size和size域），按大小区间fastbins一共有10个bin（64位下每个bin大小差0x10），遵循FILO（后进先出）

**smallchunk**：有fd和bk指针，大小小于0x400，按大小区间分成了62个bin（64位下每个bin大小差0x10），遵循FIFO（先进先出）

**largechunk**：有fd_nextsize、bk_nextsize、fd和bk指针，大小大于0x400，

一共有63个bin，分为6组，每组公差依次递增：
...
bin当中chunk从大到小排序，相同大小的按先后顺序，遵循FILO（后进先出）

**unsortedchunk**：有fd和bk指针，大小不限，只有一个bin，遵循FIFO（先进先出）

操作系统内存布局：

![](/images/fca49ea609fdce7bed7673a96ae75cf3.png)
